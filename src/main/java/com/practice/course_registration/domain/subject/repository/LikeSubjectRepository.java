...
public interface LikeSubjectRepository extends JpaRepository<LikeSubject, Long> {
  ...
  @Query("""
      ...
      FROM LikeSubject ls
      WHERE ls.memberId = :memberId AND ls.subject.id IN :subjectIds
    """)
    ...
    @Query("SELECT ls FROM LikeSubject ls JOIN FETCH ls.subject s WHERE ls.memberId = :memberId ORDER BY s.subjectName")
    Page<LikeSubject> findByMemberIdOrderBySubjectAsc(@Param("memberId") Long memberId, Pageable pageable);

    @Modifying
    @Query("DELETE FROM LikeSubject ls WHERE ls.memberId = :memberId AND ls.subject.id = :subjectId")
